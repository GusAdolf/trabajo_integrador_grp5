...
const BookingReview = () => {
  ...
  // Método de pago y datos asociados
  ...
  // Simulación de login (puedes implementar la lógica real)
  ...
  // Función para eliminar el producto de la reserva
  ...
  const handleConfirmBooking = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          // Verificamos si hay token (sesión activa)
          ...
          // Buscamos la disponibilidad que coincide con la fecha seleccionada
          ...
          // Armamos el payload para la reserva
          const bookingData = {
          ...
          // Llamamos al servicio de creación de reserva
          const bookingResponse = await createBooking(bookingData);

          if (bookingResponse) {
            // En lugar de redirigir, se abre el modal de confirmación de reserva
  ...
  // Si no hay producto, se muestra un mensaje de error
  if (!product) {
    return (
  ...
  return (
      <Box sx={{ width: '90%', margin: '0 auto', mt: 4 }}>
        ...
        <Grid container spacing={2}>
          ...
          {/* Columna Derecha: Datos del Usuario y Método de Pago */}
